...
if (message.content.toLowerCase().startsWith(prefix + `new`)) {
    ...
    if (!message.guild.roles.exists("name", "Ritual Crew")) return message.channel.send(`This server doesn't have a \`Ritual Crew\` role made, so the ticket won't be opened.\nIf you are an administrator, make one with that name exactly and give it to users that should be able to see tickets.`);
    if (message.guild.channels.exists("name", "ticket-" + message.author.id)) return message.channel.send(`You already have a ticket open.`);
    message.guild.createChannel(`ticket-${message.author.id}`, "text").then(c => {
        let role = message.guild.roles.find("name", "Ritual Crew");
